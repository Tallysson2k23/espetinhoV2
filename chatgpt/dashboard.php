...
function abrirPDV(mesa_id){
    ...
    .then(data=>{
        ...
        window.pedidoAtual = data.pedido_id;
        ...
        .then(html=>{
            ...
            setTimeout(function(){
                if(typeof carregarCarrinhoPDV === "function"){
                    carregarCarrinhoPDV();
                    window.pdvInterval = setInterval(carregarCarrinhoPDV, 2000);
                }
            },200);
...
function atualizarContador(){
    ...
    .catch(err=>{
        console.log("Erro atualizarContador:", err.message);
    });
...
window.abrirGrupoPDV = function(grupo_id){
    ...
    let mesa_id = document.getElementById("pdvTitulo")
...
window.abrirModalProduto = function(id, nome){
...
window.confirmarProduto = function(){
    ...
    fetch("api/adicionar_item.php",{
        ...
        body:new URLSearchParams({
            mesa_id: mesa_id,
            produto_id: window.produtoSelecionado,
            quantidade: qtd,
            observacao: obs
        })
    })
...
window.fecharPDV = function(){
    ...
    if(window.pdvInterval){
        clearInterval(window.pdvInterval);
    }
